...
Directory? _findSnapshots() {
  ...
  final candidates = [
    Directory('${root.path}/../tmpprobe/live'),
...
void main() {
  ...
  // 页面快照目录是本机开发暂存产物（tmpprobe/live），CI / 干净检出没有 → 整组 skip。
  final snap = _findSnapshots();
  final skip = snap == null ? '缺少本机页面快照目录 tmpprobe/live' : false;
  setUpAll(() async {
    if (snap == null) return;
  ...
  group('本机快照端到端', () {
    ...
    test('wche_dm 16dns list page parses cards', () async {
      ...
      expect(first.name.contains('&#'), isFalse, reason: '标题不应残留 HTML 实体');
      expect(
        first.pic,
        startsWith('http'),
    ...
    test('wche_dm 16dns detail cleans rating and parses episodes', () async {
      ...
      expect(
        RegExp(r'\d+\.\d+$').hasMatch(d.video.name),
        isFalse,
    ...
    test('wche_dm 16dns playUrl strips 166 prefix and pulls m3u8', () async {
      ...
      expect(url, startsWith('https://'), reason: '应补全为绝对 URL，实际：$url');
      expect(url.contains('.m3u8'), isTrue, reason: '应返回 m3u8 直链，实际：$url');
    ...
    test(
      'ashan_yy dainyew detail cleans rating and expands multi-season',
      () async {
        final src = DslVideoSource(_ashan());
    ...
    test('ashan_yy dainyew playUrl unescapes backslashes in m3u8', () async {
      ...
      expect(url.contains(r'\/'), isFalse, reason: '不应残留转义斜杠，实际：$url');
      expect(url.contains('.m3u8'), isTrue, reason: '应返回 m3u8 直链，实际：$url');
    });
  }, skip: skip);
}
