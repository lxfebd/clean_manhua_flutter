...
- 验收清单：
  ...
  - **插帧（AI 视频补帧）子项**——调研完成（2026-09-12，`docs/frame-interpolation-research.md`）：RIFE 模型（MIT）+ ncnn-vulkan 推理（Android 可复用同一推理代码走 maven AAR）；**完全落在现有 CapabilityPlugin 契约内，不改框架**（id `ai.frame.rife`、category `video`、builtin:false、artifact=桌面 .dll/.so 直链 + Android maven、weights=模型文件走 .model_cache/）。里程碑 **F0 立项待用户确认** → F1 桌面推理 PoC（ncnn 自编译 .dll + RIFE 模型转换 + Isolate 单帧补帧）→ F2 离线补帧导出 → F3 实时补帧（限分辨率档+失败降级）→ F4 Android（暂缓）。**风险**：ncnn 无官方预编译需自编译；补帧对大位移镜头有伪影；实时补帧桌面 10–50ms/帧吃紧首版限 ≤720p。
